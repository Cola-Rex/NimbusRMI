...
public class ServiceConsumer {
	...
	//定义一个 volatile 成员变量，用于保存最新的 RMI 地址（考虑到该变量或许会被其他线程所修改，一旦修改后，该变量的值会影响到所有线程
	...
	public <T extends Remote> T lookup() {
		...
		if (size > 0) {
			...
			if (size == 1) {
				...
				url = urlList.get(ThreadLocalRandom.current().nextInt()); // 若 urlList 中存在多个元素，则随机获取一个元素
	...
	private <T> T lookupService(String url) {
		...
		try {
			...
			if (e instanceof ConnectException) {
				// 若连接中断，则使用 urlList 中第一个 RMI 地址来查找 （这是一种简单的重试方法， 确保不会抛出异常）
